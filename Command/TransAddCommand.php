...
class TransAddCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        // Process
        if ($domainfiles['path'] == '') {
            $output->writeln('TRANS:ADD => ERROR : Default locale file "' . $domainfiles['filename'] . '" required and not found. Run "trans:create ' . $input->getArgument('domain') . '" command to solve it.');
            die();
            ...
            $replytarget = "";
            while ($a <= 0) {
                $question = new Question('TRANS:ADD => QUESTION : Please, enter an uniq ID that will be used for identify this translation message in all locale files : ', '');
                $replytarget = $helper->ask($input, $output, $question);
                if (!$replytarget == '' && is_string($replytarget)) {
                    if (!isset($defaultdata[$replytarget])) {
                        $a = 1;
                        $output->writeln('TRANS:ADD => INFO : The ID you have entered is OK! ');
                    } else {
        ...
        $output->writeln('TRANS:ADD => SUCCESS : Translation message saved!');
    }
}